...
class word2vec(object):
    ...
    def _forward_backward(self, x, y, lr, K):
        # forward
        ...
        # backward
        samples = self._negsample(y, K)
        delta_w = (outputprobs - 1.0) * self.weight[y]
        ...
        for idx in samples:
            ...
            delta_w += tmpoutput * self.weight[idx]
            self.weight[idx] -= lr*delta_w*inputx
        self.lookuptable[x] -= lr*delta_w
        return loss

    def _negsample(self, y, K):
        res = [None]*K
        for idx in range(K):
    ...
    def _train_skipgram(self, traindata, epochs, lr, K, vecsize):
        ...
        for epoch in range(1, epochs+1):
            ...
            for x, y in traindata:
                loss += self._forward_backward(x, y, lr, K)
                if itertimes % 1000 == 0:
                    stptime = time.time()
                ...
                if itertimes % 500000 == 0:
                    pickle.dump(self.lookuptable, open(r'.\data\wordvectors.pkl', 'wb'))
                itertimes += 1

    def _cheak(self, shownum, k):
        if not os.path.exists(r'.\data\wordvectors.pkl'):
            raise ValueError('No word vectors found!')
        self.lookuptable = pickle.load(open(r'.\data\wordvectors.pkl', 'rb'))
...
if __name__ == "__main__":
    ...
    # w2v._train_skipgram(traindata=w2v.traindata, epochs=5, lr=0.001, K=20, vecsize=100)
    # show res
    w2v._cheak(10, 5)